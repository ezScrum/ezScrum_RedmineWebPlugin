Ext.ns('Plugin.Redmine');

var redmineFormPanelID;

var redminePluginSaveBtnHandler = function(btn) {
	function showResult(btn,text){
	    if (btn == 'yes') {
	    	var formPanel = Ext.getCmp(redmineFormPanelID);
    ...
    Ext.MessageBox.confirm('Confirm', 'Your change will be applied in Product backlog.', showResult );
}


var redminePluginSaveBtn = new Ext.Button({
    text    : 'Save',
    handler : redminePluginSaveBtnHandler
});

Plugin.Redmine.PluginConfigPage = Ext.extend(Ext.Panel, {
	init: function(cmp) {
		this.hostCmp = cmp;
			  	    	   ...
			  	    	   callback: function(opt,success,resp) {
			  	    	    	for (var i in result.data.plugin) {
			  	    	    		if (result.data.plugin[i].name != "" ) {
			  	    	    			if (result.data.plugin[i].name != "redminePlugin") {
			  	    	    				continue;
			  	    	    			}
			  	    	    			redmineFormPanelID = result.data.plugin[i].name;
...
Ext.preg('redmine_ConfigPage', Plugin.Redmine.PluginConfigPage);
